...
public class CrazyOptionals {
    ...
    public static Optional<BigDecimal> retrieveBalance(AccountProvider accountProvider) {
        return Optional.ofNullable(accountProvider)                // Optional<AccountProvider>
                .flatMap(AccountProvider::getAccount)       // Optional<Account>
                .map(Account::getBalance);              // Optional<BigDecimal>
    }
    ...
    public static Optional<BigDecimal> retrieveCreditBalance(CreditAccountProvider accountProvider) {
        return Optional.ofNullable(accountProvider)                  // Optional<CreditAccountProvider>
                .flatMap(CreditAccountProvider::getAccount)   // Optional<CreditAccount>
                .flatMap(CreditAccount::getCreditBalance);    // Optional<BigDecimal>
    }
    ...
    public static Account getAccountWithMaxBalance(List<Account> accounts) {
        return accounts.stream()
                .max(Comparator.comparing(Account::getBalance))  // numeric comparison
                .orElseThrow(NoSuchElementException::new);
    }
    ...
    public static OptionalDouble findMinBalanceValue(List<Account> accounts) {
        return accounts.stream()
                .map(Account::getBalance)           // BigDecimal   ↘︎
                .mapToDouble(BigDecimal::doubleValue) // DoubleStream
                .min();
    }
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream()
                ...
                .flatMap(Optional::stream)                   // Stream<BigDecimal>
                .mapToDouble(BigDecimal::doubleValue)        // convert to primitive double
                .sum();                                      // sum the doubles
    }
}